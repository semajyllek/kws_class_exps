...
class SyntheticDatasetConfig:
    ...
    # Required: Keywords to generate
    keywords: List[str] = None
    
    # Generation parameters
    samples_per_keyword: int = 500
    tts_engine: str = 'gtts'  # 'gtts' (fast, robotic) or 'bark' (slow, natural)
    ...
    measure_energy: bool = False  # If True, measure GSC energy before generation
    target_rms: Optional[float] = None  # Auto-loaded from energy profile if None
    energy_profile_path: str = None
    
    # Audio parameters (advanced - usually don't need to change)
    ...
    def __post_init__(self):
        ...
        if self.dataset_name is None:
            # Auto-name based on engine
            self.dataset_name = f'gsc_synthetic_{self.tts_engine}'
        
        if self.energy_profile_path is None:
            self.energy_profile_path = os.path.join(
                self.output_dir,
...
class SyntheticDatasetGenerator:
    ...
    def generate_complete_dataset(self) -> str:
        ...
        logger.info(f"Target: {self.config.samples_per_keyword} samples × "
                   f"{len(self.config.keywords)} keywords")
    ...
    def _save_quality_report(self, metadata: List[Dict]):
        ...
        report = {
            ...
            'total_requested': len(self.config.keywords) * self.config.samples_per_keyword,
            'success_rate': len(metadata) / (len(self.config.keywords) * self.config.samples_per_keyword),
...
def generate_comprehensive_dataset(keywords: List[str], samples_per_keyword: int,
    ...
    """Generate comprehensive synthetic dataset (legacy CLI function)."""
...
def generate_from_config(config: SyntheticDatasetConfig) -> str:
    """Generate comprehensive synthetic dataset from config."""
    generator = SyntheticDatasetGenerator(config)
    return generator.generate_complete_dataset()
